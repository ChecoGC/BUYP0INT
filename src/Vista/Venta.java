...
public class Venta extends javax.swing.JInternalFrame {
    ...
    public String agregarProducto(String productoSeleccionado, String cantidadTexto) {
        if (productoSeleccionado.equalsIgnoreCase("Seleccione producto:")) {
            return "Seleccione un producto";
        }
        if (cantidadTexto.isEmpty()) {
            return "Ingrese la cantidad de producto";
        }
        ...
        if (cantidad <= 0) {
            return "La cantidad no puede ser cero ni negativa";
        }

        this.datosDelProducto(); // carga idProducto, nombre, precioUnitario, cantidadProductoBBDD, etc.

        int cantidadEnLista = 0;
        for (DetalleVenta p : listaProductos) {
            if (p.getIdProducto().equals(idProducto)) {
                cantidadEnLista = p.getCantidad();
                break;
            }
        }
        int cantidadTotal = cantidadEnLista + cantidad;
        if (cantidadTotal > cantidadProductoBBDD) {
            return "La cantidad total supera el stock disponible";
        }
        ...
        for (DetalleVenta p : listaProductos) {
            if (p.getIdProducto().equals(idProducto)) {
                ...
                p.setSubtotal(Math.round(nuevoSubtotal * 100) / 100.0);
                double nuevoTotal = nuevoSubtotal + iva + descuento;
                p.setTotalAPagar(Math.round(nuevoTotal * 100) / 100.0);
        ...
        if (!productoExistente) {
            ...
            producto = new DetalleVenta(auxIdDetalle, 1, idProducto, nombre, cantidad, precioUnitario, subtotal, descuento, iva, totalPagar, 1);
            listaProductos.add(producto);
            auxIdDetalle++;
        }

        return "OK"; // Indica que todo fue bien
    }
    ...
    private void btnAnadirProductoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAnadirProductoActionPerformed
        ...
        String resultado = agregarProducto(combo, cantidadTexto);
        if (resultado.equals("OK")) {
            JOptionPane.showMessageDialog(null, "Producto agregado o actualizado");
            txtCantidad.setText("");
            this.cargarProductos();
    ...
    private void btn_CalcularActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_CalcularActionPerformed
        if (!txt_Efectivo.getText().isEmpty()) {
            ...
            if (validacion) {
                ...
                if (calcularCambio(efc, top) < 0) {
                    JOptionPane.showMessageDialog(null, "El dinero en efectivo no es suficiente");
                } else {
                    String camb = String.valueOf(calcularCambio(efc, top));
